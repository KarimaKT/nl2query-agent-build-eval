...
These are markdown instruction files. Load them into any AI assistant that supports custom skills or instruction files — the folder structure is up to your tooling. The `~/.copilot/m-skills/` path shown in the install script below is one example used by some Microsoft AI tooling.
...
Get-ChildItem ".\skills" -Directory | ForEach-Object {
    $dest = "$env:USERPROFILE\.copilot\m-skills\$(# NL2Query Agent Labs for Copilot Studio
...
.Name)"  # adjust this path for your tooling
    New-Item -ItemType Directory -Path $dest -Force | Out-Null
    Copy-Item "$($_.FullName)\SKILL.md" "$dest\SKILL.md" -Force
}
```
